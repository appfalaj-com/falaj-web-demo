-- Falaj Web - allow decimal order volume values.
-- Scope:
-- 1. Convert orders.volume_liters to numeric when the column exists.
-- 2. Recreate create_order_cash_atomic with numeric order volume accumulation.
...
begin
  if exists (
    ...
    alter table public.orders
      drop constraint if exists orders_volume_liters_check;

    alter table public.orders
      add constraint orders_volume_liters_check
